...
-- Docker or pgTAP: paste the whole file into the Supabase dashboard SQL Editor
-- and run it. It seeds synthetic identities, proves each isolation rule, then
-- ROLLS BACK, so no test data persists. Any failure aborts with a FAIL message.
--
-- Safe on staging only. Do not run against a project holding real data until you
-- have read it: it creates and removes temporary auth.users rows.
...
insert into app_destinations (id, organization_id, code, label, platform, kind, manual_only, requires_approval) values
  ...
  ('a3333333-0000-4000-8000-0000000000s1','aaaaaaaa-0000-4000-8000-00000000000a','education','Edu','instagram','satellite',false,true);

insert into app_social_accounts (id, organization_id, destination_id, platform, handle, token_secret_ref) values
  ('a4444444-0000-4000-8000-000000000001','aaaaaaaa-0000-4000-8000-00000000000a','a3333333-0000-4000-8000-0000000000s1','instagram','@edu_a','secretref://vault/token-a');
...
values ('a5555555-0000-4000-8000-0000000000f1','aaaaaaaa-0000-4000-8000-00000000000a',null,'grid','a3333333-0000-4000-8000-0000000000f1','feed','dm_acquire',true,'approved');
...
begin
  ...
  execute 'set local role authenticated';
  perform set_config('request.jwt.claims', json_build_object('sub', '22222222-0000-4000-8000-000000000001')::text, true);
  ...
  execute 'set local role authenticated';
  perform set_config('request.jwt.claims', json_build_object('sub', analyst_a)::text, true);
  ...
  execute 'set local role authenticated';
  perform set_config('request.jwt.claims', json_build_object('sub', reviewer_a)::text, true);
  ...
  begin
    execute 'set local role authenticated';
    perform set_config('request.jwt.claims', json_build_object('sub', operator_a)::text, true);
  ...
  -- 8. unauthenticated (anon) sees zero tenant rows
  execute 'set local role anon';
  select count(*) into n from app_pieces;
  execute 'reset role';
  if n <> 0 then raise exception 'FAIL 8: anon read % pieces (must be 0)', n; end if;
