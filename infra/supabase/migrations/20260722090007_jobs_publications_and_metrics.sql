...
create table app_job_runs (
  ...
  gpu_seconds numeric(10,3) check (gpu_seconds >= 0),
...
create table app_publish_jobs (
  ...
  state text not null default 'pending'
    check (state in ('pending','claimed','uploading','processing','published',
...
create table app_metric_observations (
  ...
  views integer check (views >= 0), impressions integer check (impressions >= 0),
  reach integer check (reach >= 0), saves integer check (saves >= 0),
  likes integer check (likes >= 0), comments integer check (comments >= 0),
  shares integer check (shares >= 0), clicks integer check (clicks >= 0),
  profile_visits integer check (profile_visits >= 0),
  dms_attributed integer check (dms_attributed >= 0),
