...
export function createRemoteFactoryClient({config=REMOTE_FACTORY_CONFIG,fetchImpl=globalThis.fetch,session=null}={}){
  if(!config.enabled){
  ...
  const headers=extra=>({apikey:config.anonKey,Authorization:`Bearer ${token}`,...extra});
  ...
  const queue=(filters={})=>{
    ...
    return get('/rest/v1/content_items?'+params.toString());
  };
  ...
  return Object.freeze({enabled:true,mode:'remote',hasSession:!!session?.access_token,queue,sync:syncCampaign,syncCampaign});
}
